class JokeController < ApplicationController
  ...
  def fetch
    ...
    send_joke_body = {"type" => "chat", "body" => fetched_joke['joke']}
      ...
      Rails.logger.debug "What we're sending is #{HTTParty.post("https://driftapi.com/conversations/#{message_id}/messages", headers: send_joke_headers, body: send_joke_body.to_json)}"
  end
end